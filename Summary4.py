...
#                                                 데이터 불러오기
#######################################################################################################################
df = pd.read_csv("data/diabetes.csv", encoding='CP949')
print(df.shape)
# 수동으로 X 컬럼 선정하기
# print(df.columns)
...
#######################################################################################################################
#                                           Graph 형태로 시각화 - distplot
#######################################################################################################################
# 1-1) 시각화 해보기
cols = df.columns[:-1].tolist()
print(cols)
df_0 = df[y == 0]  # OK->0 대상만 모아서 DataFrame 형성
df_1 = df[y == 1]  # NG->1 대상만 모아서 DataFrame 형성
...
for i, col_name in enumerate(cols[:-1]):
    ...
    sns.distplot(df_0[col_name], ax=axes[row][col])   # 막대 그래프 간격 조정 : bins = 10
    sns.distplot(df_1[col_name], ax=axes[row][col])
# plt.show()
plt.savefig("feature_distplot.png")
plt.close()

#######################################################################################################################
#                                           Graph 형태로 시각화 - Heatmap
#######################################################################################################################

plt.figure(figsize=(20, 15))
# plt.tight_layout()
# plt.subplots_adjust(left = 1, bottom = 1, right = 1, top = 1, hspace = 0, wspace = 0)
df_corr = df.corr()
mask = np.zeros_like(df_corr, dtype=np.bool)
mask[np.triu_indices_from(mask)] = True  # 삼각형 마스크 생성
sns.set(font_scale=1.5)  # X,Y, 범례의 글씨 크기
heatmap = sns.heatmap(df_corr,annot=True,vmax=1, vmin=-1,
                      cmap="coolwarm",
                      mask = mask,
                      annot_kws={"size": 20})
# plt.show()
plt.savefig("feature_corr_heatmap.png", dpi=100)
plt.close()
...
sns.set(font_scale=1)  # X,Y, 범례의 글씨 크기
...
graph.write_png('feature_tree_picture.png')

#######################################################################################################################
#                                           RandomizedSearchCV 심화
#######################################################################################################################
